...
document.addEventListener('DOMContentLoaded', function () {
  ...
  /* ===========================================================
     4. SITE ZOOM SYNC
     Detect browser zoom and persist it so all pages adapt uniformly.
     =========================================================== */

  const ZOOM_STORAGE_KEY = 'whiskersSiteZoom';
  const ZOOM_SYNC_THRESHOLD = 0.01;
  ...
  function applyStoredZoom() {
    ...
    if (!storedZoom || Math.abs(storedZoom - currentZoom) < ZOOM_SYNC_THRESHOLD) {
      return;
    }

    const compensation = storedZoom / currentZoom;
    ...
    if (typeof document.body.style.zoom !== 'undefined') {
      ...
      document.body.style.width = `${100 / compensation}%`;
    }
  }

  function saveBrowserZoom() {
    const currentZoom = getBrowserZoom();
    localStorage.setItem(ZOOM_STORAGE_KEY, currentZoom.toString());
  }

  function initializeZoomSync() {
    const storedZoom = getStoredZoom();
    const currentZoom = getBrowserZoom();
    ...
    window.addEventListener('resize', function () {
      const newZoom = getBrowserZoom();
      const stored = getStoredZoom();

      if (Math.abs(newZoom - stored) > ZOOM_SYNC_THRESHOLD) {
        saveBrowserZoom();
      }
    });
